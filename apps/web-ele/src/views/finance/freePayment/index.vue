...
import { deleteFreePaymentById, getFreePaymentList } from '#/api';
...
//   rights.value = [...useMenuRights(path)];
//   console.warn('rights', rights.value);
// });
const [Grid, gridApi] = useVbenVxeGrid({
    ...
    proxyConfig: {
      ...
      ajax: {
        query: async ({ page, sort }, formValues) => {
          return await getFreePaymentList({
            ...formValues,
  ...
  } as VxeTableGridOptions<FreePaymentApi.FreePayment>,
  gridEvents: {
    // cellClick: ({ row }) => {
  ...
  } as VxeGridListeners<FreePaymentApi.FreePayment>,
});
...
function onDelete() {
  const selectData = gridApi.grid.getRadioRecord();
  useSelectedRow(selectData)((data) => {
    deleteFreePaymentById({ id: data.id }).then(() => {
      gridApi.query();
    });
  }, true);
}
